...
### Changelog
- **0.14.1** (2026-06-22) — **Leads Report 14-day series: Active / Conv. Rate Active columns now filled.** They previously rendered "—" because `RUNNING` was treated as a snapshot-only value. New `getActiveSeries(dates, productId?)` in `lib/report.ts` reconstructs each past WIB day from real participant timestamps — a participant is **Active** on day `d` when their RUNNING window covers `d` (window opens at `startAt` = initial proof / challenge day 1, closes at `finalSubmittedAt`/`updatedAt`, or stays open while still `RUNNING`); per-day **Conv. Rate Active** = active ÷ PAID purchases cumulative as of `d`. Derived from real timestamps (not fabricated). Pure `activeForDay(windows, paidDays, d)` extracted + unit-tested. `getReport` merges the series into each `DayMetrics`; `LeadsReport.tsx` renders the values; the TOTAL footer keeps "—" (point-in-time counts don't sum). No schema/env/migration change — image rebuild only. §20.4.
...
- **Active** and **Conv. Rate Active** = participants in the **challenge program** (§21). As of **0.14.0
  ...
  the real-time KPI cards. The **14-day series table also fills these columns** (2026-06-22) via a per-day
  reconstruction from real participant timestamps (`getActiveSeries`), not "—". §20.4.
...
| **Active** | **Real-time KPI card** = live snapshot (D6): `count(ChallengeParticipant)` with `status = RUNNING` (scoped via the participant's challenge → product when `programId` is set). **14-day series column** (2026-06-22) = per-day reconstruction (`getActiveSeries`): a participant counts on WIB day `d` when their RUNNING window covers `d` — the window opens at `startAt` (initial proof = challenge day 1) and closes at `finalSubmittedAt`/`updatedAt`, or stays open while the participant is still `RUNNING`. Derived from real timestamps, not fabricated. |
| **Conv. Rate Active** | `Active / cumulative PAID orders`, `→ 0%` when there are no purchases. KPI card uses all-time cumulative purchases; the 14-day column uses purchases cumulative **as of that day**. |

> **Active / Conv. Rate Active: live snapshot on the KPI card, reconstructed per-day in the series.**
> A participant's `RUNNING` status is current state, so the "Ringkasan Hari Ini (Real Time)" cards show a
> single current number (`ReportData.snapshot`). For the 14-day table, `getActiveSeries` rebuilds each past
> day from real participant timestamps (`startAt` → window open; `finalSubmittedAt`/`updatedAt` → close;
> still-`RUNNING` = open) — no per-day status history is stored, but the window endpoints are real, so this
> is a faithful reconstruction, not a fabricated value. The pure `activeForDay()` is unit-tested. The TOTAL
> footer row keeps "—" for both columns because per-day point-in-time counts are not additive. Snapshot
> wired in D6 (2026-06-22, resolves open Q#15); series columns filled 2026-06-22.
