...
3. **Exactly-once delivery (per item)**: one `Delivery` row per order (`orderId` unique); within it one
   ...
   (§25):** the e-book is delivered **per the active engine** — **Fonnte** → a download-LINK message
   (`SENT` = link sent; file fetched later via `/api/download/[token]`, re-downloadable); **WAHA** → a file
   ATTACHMENT (`SENT` = file sent). Attachment items `SENT` = the file was sent (always file attachments).
...
**Built, pending deploy: D16 E-book delivery — link (Fonnte) / attachment (WAHA)** (§25) — **engine-aware**
(0.19.2): Fonnte → WA text with a `/download/<token>` link (avoids the 10 MB cap); WAHA → file attachment
(base64). Attachments always sent as files. Buyer enters their registered WhatsApp number on the public page → exact match → PDF streams.
