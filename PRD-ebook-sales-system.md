...
- **Active** and **Conv. Rate Active** = participants in the **challenge program** (§21). As of **0.14.0
  ...
  Conv. Rate Active = Active ÷ cumulative PAID orders (program-scoped). They are a **live snapshot**
  shown on the real-time KPI cards (the 14-day series table keeps "—" — no per-day status history). §20.4.
...
| **Active** | **live snapshot (D6, not day-bucketed):** `count(ChallengeParticipant)` with `status = RUNNING` (scoped via the participant's challenge → product when `programId` is set). A current-state count, so it appears only on the **real-time KPI card**, not as a per-past-day column (the 14-day table keeps "—"). |
| **Conv. Rate Active** | **live snapshot:** `Active / cumulative PAID orders` (all-time, in scope), `→ 0%` when there are no purchases. Real-time KPI card only. |

> **Active / Conv. Rate Active are a live snapshot, not a daily series.** A participant's `RUNNING`
> status is current state, not a dated event — we don't store per-day status history — so these two are
> a single current number (`ReportData.snapshot`), shown on the "Ringkasan Hari Ini (Real Time)" KPI
> cards. The 14-day table's Active / Conv. Rate Active columns intentionally render "—". Wired in D6
> (2026-06-22); resolves open Q#15.
